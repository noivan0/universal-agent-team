...
### 🧠 Memory Layer (크로스런 지식 축적)

에이전트가 실행을 거듭할수록 **과거 경험에서 학습**하여 품질이 개선됩니다.

| 컴포넌트 | 역할 |
|---------|------|
| **Observer Agent** | 각 워크플로우 단계 완료 후 구조화된 사실(fact) 추출 (3개 병렬) |
| **Search Agent** | 워크플로우 시작 전 유사 프로젝트 크로스런 컨텍스트 로드 (3개 병렬) |
| **Evaluator Agent** | 독립적 4-기준 루브릭 점수 평가 → 기준 미달 시 개발 에이전트 재실행 트리거 |
| **MemoryManager** | LocalBackend(JSON) / SupermemoryBackend 자동 선택 싱글톤 |

**저장되는 지식 카테고리**:
- `bug_pattern` — 반복되는 버그 패턴 및 해결 방법
- `success_pattern` — 성공한 아키텍처/기술 조합
- `tech_decision` — 기술 스택 선택 근거
- `user_preference` — 사용자 선호 패턴
- `quality_metric` — 품질 지표 이력

**백엔드 선택**:
- `SUPERMEMORY_API_KEY` 환경변수 설정 시 → SupermemoryBackend (클라우드)
- 미설정 시 → LocalBackend (`~/.claude/memory`, JSON, 의존성 없음)
- 기억 보존 기간: 90일 (설정 가능)
...
✅ **🆕 Memory Layer** (크로스런 학습 — bug/success/tech 패턴 축적)
✅ **🆕 Observer Agent** (단계별 사실 추출 → 메모리 저장)
✅ **🆕 Search Agent** (유사 프로젝트 과거 경험 사전 로드)
✅ **🆕 Evaluator Agent** (4-기준 루브릭 독립 평가 → 자동 재실행)
...
- **Memory Layer**: Cross-run learning with Observer/Search/Evaluator agents — accumulates bug patterns, success patterns, and tech decisions across runs
...
✅ **🆕 Memory Layer** (Cross-run learning — bug/success/tech patterns)
✅ **🆕 Observer Agent** (Extract structured facts after each phase)
✅ **🆕 Search Agent** (Load past experience before each run)
✅ **🆕 Evaluator Agent** (4-criterion rubric scoring → auto re-run on failure)
