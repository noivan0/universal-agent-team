...
class ArtifactManifest(BaseModel):
    ...
    def register_artifact(self, name: str, artifact_meta: ArtifactMetadata):
        ...
        NOTE: Not used by the production orchestrator; retained because
        test_artifact_schemas.py and related test suites exercise this API
        directly.  Removing it would break 15+ test cases.
        """
    ...
    def get_artifact_info(self, name: str) -> Optional[ArtifactMetadata]:
        ...
        NOTE: Not used by the production orchestrator; retained for the same
        reason as register_artifact() — test coverage depends on it.
        """
...
class ArchitectureArtifacts(BaseModel):
    ...
    # NOTE: design_system and deployment_templates are not populated by the
    # production orchestrator.  They are preserved because test suites reference
    # them via ArchitectureArtifacts directly.  Do not remove without updating tests.
...
class DevelopmentArtifacts(BaseModel):
    ...
    # NOTE: types and tests are not written by the production dev agents.
    # Retained for test coverage (DevelopmentArtifacts field access in test suites).
...
class TestingArtifacts(BaseModel):
    ...
    # NOTE: restart_plan is not populated by the production QA agent.
    # Retained for test coverage (TestingArtifacts schema tests).
...
class DocumentationArtifacts(BaseModel):
    ...
    # NOTE: user_guide is not generated by the production Documentation agent.
    # Retained for test coverage (DocumentationArtifacts field tests).
...
class AgentState(BaseModel):
    ...
    messages: List[AgentMessage] = Field(
        ...
        max_length=10000,  # Reasonable max for long workflows
        description="Agent execution messages (max 10000)"
    )
    errors: List[AgentError] = Field(
        default_factory=list,
        max_length=1000,  # Keep errors more limited
    ...
    def add_message(self, message: AgentMessage):
        ...
        if not isinstance(self.messages, deque):
            # Convert to bounded deque on first mutation for O(1) eviction
            object.__setattr__(self, 'messages', deque(self.messages, maxlen=10000))
    ...
    def add_error(self, error: Union[str, "AgentError"]):
        ...
        """
        if not isinstance(self.errors, deque):
            # Convert to bounded deque on first mutation for O(1) eviction
            object.__setattr__(self, 'errors', deque(self.errors, maxlen=1000))
...
def apply_state_update(state: AgentState, update: StateUpdate) -> AgentState:
    ...
    logger.debug("State update applied successfully")
    return state
